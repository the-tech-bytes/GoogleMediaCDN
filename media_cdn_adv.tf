...
resource "google_network_services_edge_cache_origin" "instance1" {
  name           = "media-cdn-origin1"
  origin_address = "mediacdn.origin1.example.net"
  description    = "The origin for Media CDN VOD Content"
  project      = <Enter your project number>
  protocol     = "HTTP2"
  port         = 443
  ...
  routing {
    ...
    path_matcher {
      ...
      route_rule {
        ...
        origin = google_network_services_edge_cache_origin.instance1.id
        route_action {
          url_rewrite {
            host_rewrite = "mediacdn.origin1.example.net"
          }
          cdn_policy {
            cache_mode  = "FORCE_CACHE_ALL"
            client_ttl  = "86400s"
	...
	response_header_to_add {
            header_name = "cache-id"
            header_value = "{cdn_cache_id}"
          }
        }
      }
    }	
